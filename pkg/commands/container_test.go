...
// TestContainerAttach covers the guard conditions in Attach that reject
// attaching before the container's details are loaded, when stdin isn't open,
// or when the container has exited. On the happy path it returns the docker CLI
...
func TestContainerAttach(t *testing.T) {
	...
	inspectWith := func(openStdin bool) container.InspectResponse {
		resp := container.InspectResponse{
			ContainerJSONBase: &container.ContainerJSONBase{},
			Config:            &container.Config{OpenStdin: openStdin},
		}
		return resp
	}
	...
	cases := []struct {
		...
		{
			name:    "stdin not open",
			details: inspectWith(false),
			wantErr: tr.UnattachableContainerError,
		},
		{
			name:    "exited container",
			details: inspectWith(true),
			state:   "exited",
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			ctr := newTestContainer(t, &fakeDockerClient{})
			ctr.OSCommand = NewDummyOSCommand()
			ctr.Details = tc.details
			ctr.Container = container.Summary{State: tc.state}
